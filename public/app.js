...
function subtitleForValue(record,value){if(value===''||value==null)return null;return record?.streams?.find(s=>s.codec_type==='subtitle'&&Number(s.stream_index)===Number(value))||null}
function preferredTextSubtitle(record){const tracks=(record?.streams||[]).filter(s=>s.codec_type==='subtitle'&&isTextSubtitle(s));if(!tracks.length)return null;return tracks.sort((a,b)=>{const score=s=>(['ita','it'].includes(String(s.language||'').toLowerCase())?20:0)+(s.is_default?5:0)+(s.forced?2:0);return score(b)-score(a)})[0]}
function prewarmPreferredSubtitle(record){const track=preferredTextSubtitle(record);if(!track)return;fetch(`/api/media/${record.media.id}/subtitle/${track.stream_index}.vtt?offset=0`,{cache:'force-cache'}).catch(()=>{})}
...
async function openMedia(id,show=true){
  try{const d=await api(`/api/media/${id}`);state.record=d;const m=d.media;
    ...
    $('#subtitleSelect').innerHTML='<option value="">Off</option>'+subs.map(s=>`<option value="${s.stream_index}">${esc((s.language||'und').toUpperCase())} · ${esc((s.codec_name||'sub').toUpperCase())}${s.title?` · ${esc(s.title)}`:''}${isTextSubtitle(s)?'':' · bitmap · burn-in'}</option>`).join('');
    ...
    $('#audioSelect').onchange=()=>{if(state.active?.mediaId===id)restartAt(currentGlobal())};
    $('#subtitleSelect').onchange=()=>{if(state.active?.mediaId!==id)return;const selected=subtitleForValue(d,$('#subtitleSelect').value);if(state.active.subtitleBurned||(selected&&!isTextSubtitle(selected)))restartAt(currentGlobal());else applySubtitleSelection(d,state.active)};
    $('#speedSelect').onchange=()=>{$('#player').playbackRate=Number($('#speedSelect').value)};
    prewarmPreferredSubtitle(d);
...
function applySubtitleSelection(record=state.record,playback=state.active){const p=$('#player'),select=$('#subtitleSelect'),idx=select.value;clearSubtitleTracks();if(!playback)return;if(idx===''){playback.subtitleStreamIndex=null;return}const selected=subtitleForValue(record,idx);playback.subtitleStreamIndex=Number(idx);if(!selected||!isTextSubtitle(selected))return;const track=document.createElement('track');track.kind='subtitles';track.label=selected.title||'VELA';track.srclang=String(selected.language||'it').toLowerCase().replace('ita','it').replace('eng','en');track.default=true;const offset=playback.type==='HLS'?Number(playback.base||0):0;track.src=`/api/media/${record.media.id}/subtitle/${idx}.vtt?offset=${offset}`;const show=()=>{try{if(track.track)track.track.mode='showing'}catch{}};track.addEventListener('load',show,{once:true});track.addEventListener('error',()=>toast('Errore nel caricamento dei sottotitoli',4500),{once:true});p.appendChild(track);show()}
...
async function playMedia(id,startSeconds=0){
  ...
  try{
    ...
    const p=$('#player'),subtitleValue=$('#subtitleSelect').value;state.active={mediaId:id,sessionId:x.sessionId||null,type:x.type,base:Number(x.startSeconds||0),decision:x.decision,record,subtitleStreamIndex:subtitleValue===''?null:Number(subtitleValue),subtitleBurned:Boolean(x.decision?.subtitleAction==='BURN')};
    ...
    p.src=x.url;p.playbackRate=Number($('#speedSelect').value||1);if(state.active.subtitleBurned)clearSubtitleTracks();else applySubtitleSelection(record,state.active);
