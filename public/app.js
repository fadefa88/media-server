...
const state = {
  health:null, home:null, heroIndex:0, heroTimer:null, record:null, active:null,
  lastProgressSave:0, metadataTimer:null, searchTimer:null,
  queue:[], queueIndex:-1, postPlayTimer:null, postPlaySeconds:0
};
...
function toast(text, ms=2600){const e=$('#toast');if(!e)return;e.textContent=text;e.hidden=false;clearTimeout(toast.t);toast.t=setTimeout(()=>e.hidden=true,ms)}
...
async function boot(){
  try{
    ...
    $('#serverState').textContent=`LDF Media Server ${health.version}`;$('#serverDetail').textContent='online · Client First';
  ...
  }catch(e){if($('#serverDetail'))$('#serverDetail').textContent='offline';toast(e.message,5000)}
}

function renderHero(index=0){
  const candidates=state.home?.heroCandidates?.length?state.home.heroCandidates:[state.home?.hero].filter(Boolean);if(!candidates.length){$('#heroContent').innerHTML='<div class="kicker">LDF MEDIA SERVER</div><h1>La tua libreria.</h1><p>Scansiona i media e collega TMDB.</p>';return}
...
function bindStatic(){
  $('#systemBtn').onclick=openSystem;$('#activityBtn').onclick=openSystem;$('#metadataQuick').onclick=openSystem;$('#systemClose').onclick=()=>$('#systemSheet').close();
  ...
  $('#topHomeButton').onclick=()=>$('#homeBtn')?.click();$('#castBtn').onclick=()=>{if(!$('#playerLayer').hidden)$('#playerQueuePanel').hidden=false;else toast('Nessun player attivo')};$('#closeSearch').onclick=closeSearch;
  ...
  document.addEventListener('click',async e=>{
    const quick=e.target.closest('[data-quick-play]');if(quick)return;
...
async function openMedia(id,show=true){
  try{const d=await api(`/api/media/${id}`);state.record=d;const m=d.media;
    ...
    $('#detailKicker').textContent=m.media_kind==='tv'?'SERIE · EPISODIO':'LDF MEDIA SERVER';$('#detailTitle').textContent=m.display_title||m.filename;
    ...
    $('#detailOverview').textContent=m.overview||'Metadata TMDB non ancora disponibile.';
...
function queueFromContext(id){
  const context=Array.isArray(window.LDFPlexPlaybackContextIds)?window.LDFPlexPlaybackContextIds.map(Number).filter(Number.isFinite):[];
  if(context.includes(Number(id)))return [...new Set(context)];
  return [Number(id)];
}
function ensureQueue(id){
  const n=Number(id);if(state.queue.includes(n)){state.queueIndex=state.queue.indexOf(n);return}
  state.queue=queueFromContext(n);state.queueIndex=Math.max(0,state.queue.indexOf(n));renderQueuePanel();
}
function queueMeta(id){return window.LDFPlexMediaIndex?.[Number(id)]||{id:Number(id),title:`Media ${id}`,subtitle:'',art:null,poster:null,duration:0}}
function renderQueuePanel(){
  const host=$('#playerQueueList');if(!host)return;host.innerHTML=state.queue.map((id,index)=>{const m=queueMeta(id);return `<button class="playerQueueItem ${index===state.queueIndex?'active':''}" data-queue-index="${index}"><div class="playerQueueThumb">${m.art?`<img src="${esc(m.art)}" alt="">`:''}</div><div class="playerQueueText"><b>${esc(m.title)}</b><span>${esc(m.subtitle||fmt(m.duration))}</span></div><span class="playerQueueIndex">${index+1}</span></button>`}).join('')||'<div class="plexEmpty">Coda vuota</div>';
}
async function playQueueIndex(index){if(index<0||index>=state.queue.length)return;state.queueIndex=index;const id=state.queue[index];await playMedia(id,0,{preserveQueue:true})}
async function playNext(){if(state.queueIndex+1<state.queue.length)await playQueueIndex(state.queueIndex+1)}
async function playPrevious(){if(currentGlobal()>8)return restartAt(0);if(state.queueIndex>0)await playQueueIndex(state.queueIndex-1);else restartAt(0)}

async function playMedia(id,startSeconds=0,{preserveQueue=false}={}){
  try{
    hidePostPlay();if(!preserveQueue)ensureQueue(id);else state.queueIndex=state.queue.indexOf(Number(id));renderQueuePanel();
    ...
    $('#playerLayer').hidden=false;$('#playerBackdrop').style.backgroundImage=record.media.backdrop_url?`url('${record.media.backdrop_url}')`:'';$('#playerTitle').textContent=record.media.display_title;$('#playerSeries').textContent=record.media.title&&record.media.episode_title?record.media.title:'LDF Media Server';
    p.pause();p.innerHTML='';p.removeAttribute('src');p.load();
    if(x.type==='HLS'&&!p.canPlayType('application/vnd.apple.mpegurl'))throw new Error('Questo browser non riproduce HLS nativamente. Usa Safari/iPhone per il remux LDF.');
...
function attachSubtitle(record,playback){const p=$('#player'),idx=$('#subtitleSelect').value;if(idx==='')return;const track=document.createElement('track');track.kind='subtitles';track.label='LDF';track.srclang='it';track.default=true;const offset=playback.type==='HLS'?Number(playback.startSeconds||0):0;track.src=`/api/media/${record.media.id}/subtitle/${idx}.vtt?offset=${offset}`;p.appendChild(track)}
...
async function restartAt(seconds){if(!state.active)return;const id=state.active.mediaId;await saveCurrentProgress();await playMedia(id,Math.max(0,seconds),{preserveQueue:true})}
...
async function closePlayer(){hidePostPlay();await saveCurrentProgress();$('#player').pause();await stopSession();$('#playerLayer').hidden=true;$('#pulsePanel').hidden=true;$('#playerQueuePanel').hidden=true;await reloadHome().catch(()=>{})}
...
function updatePulse(d={}){$('#pulseDecision').textContent=d.reason||'LDF mantiene il video originale ogni volta che il client può decodificarlo.';$('#pulseVideo').textContent=`${d.videoAction||'COPY'} · ${state.record?.media?.video_codec?.toUpperCase()||'VIDEO'}`;$('#pulseAudio').textContent=d.audioAction||'COPY';$('#pulseCpu').textContent=d.cpuImpact||'MINIMAL';$('#pulseQuality').textContent=d.qualityPreserved?'ORIGINALE':(d.target||'AUTO')}

async function showPostPlay(){
  await saveCurrentProgress(true);if(state.queueIndex+1>=state.queue.length)return;
  const nextId=state.queue[state.queueIndex+1],meta=queueMeta(nextId),layer=$('#postPlay');
  $('#postPlayTitle').textContent=meta.title;$('#postPlayMeta').textContent=meta.subtitle||'Prossimo elemento nella coda';$('#postPlayArt').innerHTML=meta.art?`<img src="${esc(meta.art)}" alt="">`:'';
  layer.hidden=false;state.postPlaySeconds=10;$('#postPlayCountdown').textContent=`Riproduzione automatica tra ${state.postPlaySeconds} s`;
  clearInterval(state.postPlayTimer);state.postPlayTimer=setInterval(async()=>{state.postPlaySeconds-=1;if(state.postPlaySeconds<=0){hidePostPlay();await playNext()}else $('#postPlayCountdown').textContent=`Riproduzione automatica tra ${state.postPlaySeconds} s`},1000);
}
function hidePostPlay(){clearInterval(state.postPlayTimer);state.postPlayTimer=null;if($('#postPlay'))$('#postPlay').hidden=true}

function setupPlayerControls(){
  const p=$('#player');
  p.addEventListener('timeupdate',updateTime);
  p.addEventListener('play',()=>$('#togglePlay').innerHTML='<i class="fa-solid fa-pause"></i>');
  p.addEventListener('pause',()=>$('#togglePlay').innerHTML='<i class="fa-solid fa-play"></i>');
  p.addEventListener('ended',showPostPlay);
  $('#togglePlay').onclick=()=>p.paused?p.play():p.pause();$('#back10').onclick=()=>restartAt(currentGlobal()-10);$('#forward30').onclick=()=>restartAt(currentGlobal()+30);$('#playerPrevious').onclick=playPrevious;$('#playerNext').onclick=playNext;
  $('#timeline').addEventListener('change',()=>restartAt(Number($('#timeline').value)));$('#volumeSlider').addEventListener('input',()=>{p.volume=Number($('#volumeSlider').value)});
  $('#pipBtn').onclick=async()=>{try{if(document.pictureInPictureElement)await document.exitPictureInPicture();else if(p.requestPictureInPicture)await p.requestPictureInPicture()}catch{}};$('#airplayBtn').onclick=()=>p.webkitShowPlaybackTargetPicker?.();$('#fullscreenBtn').onclick=()=>$('#playerLayer').requestFullscreen?.();
  $('#playerAudio').onclick=()=>{if(!$('#detail').open)$('#detail').showModal();setTimeout(()=>$('#audioSelect').focus(),80)};$('#playerSub').onclick=()=>{if(!$('#detail').open)$('#detail').showModal();setTimeout(()=>$('#subtitleSelect').focus(),80)};
  $('#playerQueueBtn').onclick=()=>{$('#playerQueuePanel').hidden=!$('#playerQueuePanel').hidden;renderQueuePanel()};$('#playerQueueClose').onclick=()=>$('#playerQueuePanel').hidden=true;
  $('#playerQueueList').addEventListener('click',e=>{const item=e.target.closest('[data-queue-index]');if(item)playQueueIndex(Number(item.dataset.queueIndex))});
  $('#postPlayNow').onclick=()=>{hidePostPlay();playNext()};$('#postPlayCancel').onclick=hidePostPlay;
}
...
window.LDFPlayer={
  open:id=>openMedia(Number(id)),
  play:async id=>{const n=Number(id);ensureQueue(n);await openMedia(n,false);return playMedia(n,state.record?.media?.progress_seconds||0,{preserveQueue:true})},
  setQueue:ids=>{state.queue=[...new Set((ids||[]).map(Number).filter(Number.isFinite))];state.queueIndex=-1;renderQueuePanel()},
