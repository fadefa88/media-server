...
export function parseMediaIdentity(media) {
  ...
  if (se) {
    ...
    if (seasonIndex > 0) titleSource = parts[seasonIndex - 1];
      ...
      titleSource = String(media.filename || '').split(/\bS\d{1,2}E\d{1,4}\b|\b\d{1,2}x\d{1,4}\b|\bE\d{2,4}\b/i)[0];
    }
  }

  return {
    kind: se ? 'tv' : 'movie',
    query: cleanTitle(titleSource) || cleanTitle(media.filename || ''),
    year,
...
function chooseBest(results = [], identity) {
  ...
  for (const item of results.slice(0, 12)) {
    ...
    let score = 0;
    if (candidate === q) score += 120;
    else if (candidate.startsWith(q) || q.startsWith(candidate)) score += 70;
    else if (candidate.includes(q) || q.includes(candidate)) score += 40;
    ...
    score += Math.min(12, Number(item.vote_count || 0) / 500);
    score += Math.min(8, Number(item.popularity || 0) / 25);
...
async function lookup(identity) {
  ...
  if (identity.kind === 'tv') {
    const search = await request('/search/tv', {
      query: identity.query,
      language: LANGUAGE,
      include_adult: false,
      first_air_date_year: identity.year || undefined
    });
    const hit = chooseBest(search.results, identity);
    if (!hit) return null;
    ...
    if (identity.season !== null && identity.episode !== null) {
      try {
        episode = await request(`/tv/${hit.id}/season/${identity.season}/episode/${identity.episode}`, { language: LANGUAGE });
      } catch { /* show metadata is still useful when provider numbering differs */ }
    }
...
function metadataFrom(match, identity) {
  ...
  return {
    ...
    tmdbId: d.id,
    title: match.kind === 'movie' ? d.title : d.name,
    ...
    genres: Array.isArray(d.genres) ? d.genres.map(g => ({ id: g.id, name: g.name })) : [],
    originalLanguage: d.original_language || null,
    seasonNumber: identity.season,
    episodeNumber: identity.episode,
    episodeTitle: e?.name || null
  };
}

export async function enrichMedia(pool, media) {
  const identity = parseMediaIdentity(media);
  const match = await lookup(identity);
  if (!match) {
    await pool.query(`UPDATE media SET media_kind=$2, season_number=$3, episode_number=$4, metadata_status='MISS', metadata_error=$5, metadata_updated_at=now() WHERE id=$1`,
      [media.id, identity.kind, identity.season, identity.episode, `Nessun match TMDB per: ${identity.query}`]);
  ...
  await pool.query(`
    ...
    WHERE id=$1
  `, [media.id, m.mediaKind, m.tmdbId, m.title, m.originalTitle, m.releaseDate, m.releaseYear,
    m.overview, m.tagline, m.posterPath, m.backdropPath, m.stillPath, m.voteAverage,
    JSON.stringify(m.genres), m.originalLanguage, m.seasonNumber, m.episodeNumber, m.episodeTitle]);
  return { status: 'READY', identity, metadata: m };
}

export function createMetadataState() {
  return { configured: tmdbConfigured(), running: false, total: 0, processed: 0, matched: 0, missed: 0, errors: 0, current: null, startedAt: null, finishedAt: null, lastError: null };
}
...
export async function enrichLibrary({ pool, limit = 0, force = false, state = createMetadataState() }) {
  ...
  state.running = true; state.processed = 0; state.matched = 0; state.missed = 0; state.errors = 0;
  state.current = null; state.startedAt = new Date().toISOString(); state.finishedAt = null; state.lastError = null;
  try {
    ...
    for (const media of rows.rows) {
      ...
      try {
        const result = await enrichMedia(pool, media);
        if (result.status === 'READY') state.matched++; else state.missed++;
      } catch (error) {
